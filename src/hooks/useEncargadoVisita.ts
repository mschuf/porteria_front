...
export function useEncargadoVisitaHistory(initialFilters:Partial<Omit<EncargadoVisitaHistoryQuery,"page"|"limit">>={}){
 const [draft,setDraft]=useState<Omit<EncargadoVisitaHistoryQuery,"page"|"limit">>(()=>({...initialFilters}));const [applied,setApplied]=useState<Omit<EncargadoVisitaHistoryQuery,"page"|"limit">>(()=>({...initialFilters}));const [page,setPage]=useState(1);const [limit,setLimit]=useState(15);const [sort,setSort]=useState<Pick<EncargadoVisitaHistoryQuery,"sortBy"|"sortOrder">>({});const [items,setItems]=useState<EncargadoVisitaRecord[]>([]);const [total,setTotal]=useState(0);const [loading,setLoading]=useState(true);const [error,setError]=useState("");

/**
 * @file UsuarioEmpresaTable.tsx
 * @description Tabla de asignaciones usuario-empresa con orden por columnas.
 */
import { ArrowDown, ArrowUp, ArrowUpDown, Pencil, Power, Trash2 } from "lucide-react";
import type { UsuarioEmpresa, UsuarioEmpresaSortColumn, UsuarioEmpresaSortOrder } from "@/api/usuario-empresa";
import { Badge } from "@/components/ui/badge";
import { EmptyState } from "@/components/ui/empty-state";
import { cn } from "@/lib/utils";

interface UsuarioEmpresaTableProps {
  rows: UsuarioEmpresa[];
  sortColumn?: UsuarioEmpresaSortColumn | null;
  sortOrder?: UsuarioEmpresaSortOrder | null;
  onSortColumnChange?: (column: UsuarioEmpresaSortColumn) => void;
  onEdit: (asignacion: UsuarioEmpresa) => void;
  onActivate: (asignacion: UsuarioEmpresa) => void;
  onDeactivate: (asignacion: UsuarioEmpresa) => void;
  onDelete: (asignacion: UsuarioEmpresa) => void;
}

const SORTABLE_COLUMNS: Array<{ id: UsuarioEmpresaSortColumn; label: string }> = [
  { id: "id", label: "ID" },
...
/** Tabla ordenable de asignaciones usuario-empresa con acciones CRUD. */
export function UsuarioEmpresaTable({
  rows,
  sortColumn,
  sortOrder,
  onSortColumnChange,
  ...
  onDeactivate,
  onDelete,
}: UsuarioEmpresaTableProps) {
  if (rows.length === 0) {
    return (
      <EmptyState
        title="Sin asignaciones"
        description="No hay asignaciones usuario-empresa registradas o no coinciden con los filtros aplicados."
      />
    );
  }
          ...
          <tbody className="divide-y divide-border">
            {rows.map((asignacion) => (
              <tr key={asignacion.id} className="hover:bg-muted/40">
                ...
                <td className="px-4 py-3">{asignacion.empresaNombre}</td>
